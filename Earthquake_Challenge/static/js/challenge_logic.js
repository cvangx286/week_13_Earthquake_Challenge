...
let major_earthquake_data = new L.LayerGroup();

// 2. Add a reference to the tectonic plates group to the overlays object.
let overlays = {
  "Earthquakes": allEarthquakes,
  "Tectonic plates": tectonicPlateData,
  "Major Earthquakes": major_earthquake_data
};

// Then we add a control to the map that will allow the user to change which
// layers are visible.
L.control.layers(baseMaps, overlays,tectonicPlateData,major_earthquake_data).addTo(map);
...
}).addTo(major_earthquake_data);

// Then we add the earthquake layer to our map.
major_earthquake_data.addTo(map);
...
// Looping through our intervals to generate a label with a colored square for each interval.
  ...
  d3.json("https://raw.githubusercontent.com/fraxen/tectonicplates/master/GeoJSON/PB2002_boundaries.json").then(function(data) {
    console.log(data);
    L.geoJSON(data).addTo(tectonicPlateData);
